...
ENUM(Platform, v_int32,
    VALUE(PC, 0, "PLATFORM_PC"),
    VALUE(MAC, 1, "PLATFORM_MAC"),
    VALUE(Linux, 2, "PLATFORM_LINUX"),
    VALUE(Other, 3, "PLATFORM_OTHER")
);

ENUM(Player, v_int32,
    VALUE(Single, 0, "PLAYER_SINGLE"),
    VALUE(Multi, 1, "PLAYER_MULTI")
);

ENUM(GameType, v_int32,
    VALUE(Action, 0, "GAMETYPE_ACTION"),
    VALUE(Multi, 1, "GAMETYPE_ADVENTURE")
);
